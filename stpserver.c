...
int main(int argc, char *argv[])
{
    ...
    if (demand < available)
    {
        ...
        for (int i = 0; i < demand; i++)
        {
            ...
            {
                arr[pos] = 1;
                printf("No. of tickets booked : %d ", i + 1);
            }
            else if (arr[pos] != 1)
            {
                write(newsockfd, "Enter the row no.", sizeof("Enter the row no."));
                arr[pos] = 1;
                printf("\nNo. of tickets booked : %d \n\n", i + 1);
            }
            else
            {
    ...
    {
        write(newsockfd, "sorry, tickets unavailable", strlen("sorry, tickets unavaiable"));
        close(newsockfd);
        close(sockfd);
    }
//}
